...
int check_cache_data_hit(void *addr, char type) {
    ...
    int block_addr = *addr / DEFAULT_CACHE_BLOCK_SIZE_BYTE;
    int byte_offset = *addr % DEFAULT_CACHE_BLOCK_SIZE_BYTE;
...
int find_entry_index_in_set(int cache_index) {
    ...
    if (DEFAULT_CACHE_ASSOC == 1)
        ...
        int low_timestamp = 100000000;
        int low_timestamp_index = -1;
        int check = 0;
        for (j = 0; j < DEFAULT_CACHE_ASSOC; j++) {
            cache_entry_t* pEntry = &cache_array[cache_index][j];
            if (pEntry->valid == 0) {
            ...
            if (low_timestamp > pEntry->timestamp) {
                low_timestamp = pEntry->timestamp;
                low_timestamp_index = j;
            }
        }
        if (!check) entry_index = low_timestamp_index;
    }
...
int access_memory(void *addr, char type) {
    ...
    cache_entry_t* pEntry = &cache_array[cache_index][find_entry_index_in_set(cache_index)];
    pEntry->data = memory_array[(block_addr * 2) + (byte_offset / 4)][byte_offset % 4:(byte_offset % 4) + type_len];
    return pEntry->data;
